. $PSScriptRoot\..\SystemCleaner.ps1 -SkipBootstrap

Describe 'SystemCleaner formatting helpers' {
    ...
    It 'returns a compact ASCII logo block' {
        $logo = Get-AppLogoLines

        $logo.Count | Should BeLessThan 7
        ($logo | Measure-Object -Property Length -Maximum).Maximum | Should BeLessThan 67
        (($logo -join '')).ToCharArray() | Where-Object { [int][char]$_ -gt 127 } | Should BeNullOrEmpty
    }
}

Describe 'SystemCleaner utility functions' {
    ...
    It 'detects non-administrator in normal session' {
        # Should not crash in any execution context
        $result = Test-IsAdministrator
        $result | Should BeOfType System.Boolean
    }
}

Describe 'SystemCleaner mode colors' {
...
Describe 'SystemCleaner tracking and safety' {
